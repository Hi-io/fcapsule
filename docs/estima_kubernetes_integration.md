# Estima Integration in Kubernetes

Estima is a separately operated service, with its own repository, HTTP API and
PostgreSQL database: [Hi-io/estima](https://github.com/Hi-io/estima). This guide
covers the FCAPSule client side only. It does not install Estima or PostgreSQL
from the FCAPSule repository. Follow Estima's repository for service deployment,
database migrations, backups and lifecycle operations.

## Topology

```text
FCAPSule A -- optional HTTPS/API --> Estima API -- SQL --> PostgreSQL
FCAPSule B -- optional HTTPS/API -->       |
                                          no LLMs
```

Each FCAPSule retains its own evidence and runs its own configured model calls.
Estima only validates, stores and returns bounded case records. It has no
...
In **Settings**, set the base URL and service bearer token provided by the Estima
operator. Or supply environment defaults:

```bash
FCAPSULE_ESTIMA_URL=https://estima.example.internal
FCAPSULE_ESTIMA_TOKEN=<service-bearer-token>
FCAPSULE_ESTIMA_INSTANCE_ID=<stable-unique-instance-id>
FCAPSULE_ESTIMA_READ=false
FCAPSULE_ESTIMA_PUBLISH=false
```

Do not put the Estima token or FCAPSule provider keys in a ConfigMap. Mount
`FCAPSULE_ESTIMA_TOKEN` from the cluster's approved Secret manager. This token
authenticates FCAPSule to Estima; it is not an LLM API key. Provider keys remain
in the normal FCAPSule Secret and are never needed by Estima. The endpoint is a
placeholder: use the actual Estima address issued by its operator.

Reads and publication are independent opt-ins and are off by default. Enable one
or both only after reviewing the outgoing case projection and the Estima sharing
policy. Assign each FCAPSule instance a different stable
`FCAPSULE_ESTIMA_INSTANCE_ID`, even when instances share one endpoint and token.
Saved Settings override environment defaults. Current client settings are in
`estima-settings.json`; older state migrates values from `atlas-settings.json`
when the new file does not exist. Legacy `FCAPSULE_ATLAS_*` variables remain
fallbacks for compatibility; new deployments should use `FCAPSULE_ESTIMA_*`.

The client requires HTTPS except for localhost and Kubernetes `.svc` DNS names.
For an in-cluster Estima service, use its service DNS name and the cluster's
...
Any number of FCAPSule instances can point to the same Estima endpoint. Estima's
...
The FCAPSule repo contains no Estima Deployment, database credentials, or
PostgreSQL storage manifest. Estima's operator is responsible for selecting
durable PostgreSQL storage, backup/restore, retention, deletion and service
readiness checks. The service's `/healthz` checks Estima/database health only; it
does not establish that FCAPSule sources or provider calls are healthy.

## Verify and Operate

1. Deploy Estima using the instructions in its [repository](https://github.com/Hi-io/estima).
...
5. Test an invalid token, schema/response mismatch, slow response and full
   ...
   working; Estima failure must be distinguishable from an empty search.
6. Monitor local pending/failed publication state and Estima's independent
...
an incident locally does not delete its remote Estima copy. The current service
...
for investigation, not a verified cause. See the [Estima integration guide](estima.md)
for API, privacy, lifecycle and evaluation limits.
