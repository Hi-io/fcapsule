...
(function (global) {
  ...
  function explorer(root, options) {
    ...
    function overview() {
      if(state.phase==='error') return '<div class="em-panel-intro"><span class="em-overline">RETAINED KNOWLEDGE</span><h2>Collective is unavailable.</h2><p>The case count cannot be checked right now. Refresh to retry.</p></div>';
    ...
    function render() {
      if(disposed) return;
      graph=makeGraph();
      root.innerHTML=`<div class="em-explorer">${header()}${toolbar()}<div class="em-workspace"><div class="em-main">${state.mode==='map' ? mapMarkup() : listMarkup()}${state.mode==='map' && state.phase!=='error' ? footer() : ''}<div class="em-status" role="status" aria-live="polite">${esc(statusText())}</div></div><aside id="em-panel" class="em-panel" aria-label="Memory details">${panelContent()}</aside></div></div>`;
